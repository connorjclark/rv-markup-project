...
I went with node (JavaScript ES6, specifically), mainly because it's the environment I've used the most this past month.

I started by setting up the node project structure. I enjoy some of the new features in ES6, so I used babel. To keep it simple, I just used the preset `babel-preset-env`. I used the `standard` linting package (as opposed to writing custom eslint rules, which is usually a waste of time).
...
I used `https://github.com/mysqljs/mysql` because I didn't want to bother with a RDBMS interface. I spun my wheels a bit rolling my own migration system. Used `async` to avoid terrible callback hell.
